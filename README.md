...
**Эквивалентная схема участка почвы, на котором производится измерение, состоит из параллельно включенных резистора и конденсатора.** _Soil can be represented by a resistor and a capacitor connected in parallel._ **Резистор имеет сопротивление от сотен ом до сотен килоом, в зависимости от влажности почвы и ее солености/кислотности и т.п.** _Resistance is in the range from hundreds of ohms to hundreds of kiloohms, depending on soil moisture and its salinity / acidity, etc._ **Емкость конденсатора зависит от площади электродов, расстояния между ними, а также от влажности почвы.** _Capacitance depends on electrodes area, the distance between them, as well as on soil moisture._ **Конденсатор в эквивалентной схеме обычно имеет емкость единицы-десятки пикофарад, когда электроды находятся в воздухе.** _Usually capacitance is few picofarads when the electrodes are in air._ **Если опустить их в воду, емкость увеличивается примерно в 2-3 раза, так как диэлектрическая проницаемость воды в 80 раз больше, чем диэлетрическая проницаемость воздуха.** _In water capacitance increases by about 2–3 times, since the dielectric constant of water is 80 times greater than the dielectric constant of air._
